...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/mcodeSettingsTypes.js';
import { modelRouterSelectionFromSettings, routeModelByIntent } from '../common/modelIntentRouter.js';
...
export interface IChatThreadService {
	...
	// rollback
	performUndo(threadId: string): Promise<void>;
...
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	constructor(
		...
		@IVoidModelService private readonly _mcodeModelService: IVoidModelService,
		...
		@IGenerateCommitMessageService private readonly _generateCommitMessageService: IGenerateCommitMessageService,
		@IVoidRagService private readonly _mcodeRagService: IVoidRagService,
	...
	private _latestUserMessageText(threadId: string): string | undefined {
	...
	private _currentModelSelectionProps = (userMessage?: string) => {
		// these settings should not change throughout the loop (eg anthropic breaks if you change its thinking mode and it's using tools)
		const featureName: FeatureName = 'Chat'
		let modelSelection = this._settingsService.state.modelSelectionOfFeature[featureName]
		if (userMessage && modelSelection) {
			const routerConfig = modelRouterSelectionFromSettings(this._settingsService.state.globalSettings)
			const routed = routeModelByIntent(userMessage, modelSelection, routerConfig)
			if (routed.routed) {
				modelSelection = routed.model
			}
		}
	...
	approveLatestToolRequest(threadId: string) {
		...
		this._wrapRunAgentToNotify(
			this._runChatAgent({ callThisToolFirst, threadId, ...this._currentModelSelectionProps(this._latestUserMessageText(threadId)) })
			, threadId
		)
	}
	rejectLatestToolRequest(threadId: string) {
		...
		this._wrapRunAgentToNotify(
			this._runChatAgent({ threadId, ...this._currentModelSelectionProps(this._latestUserMessageText(threadId)) })
			, threadId
		)
	}
	...
	private _runToolCall = async (
		...
		if (!opts.preapproved) { // skip this if pre-approved
			...
			if (!isBuiltInTool) {
				const isDestructive = /(write|create|delete|remove|update|run|execute|exec|install|bash|cmd|shell|modify|patch|edit)/i.test(toolName);
				if (!isDestructive) {
					approvalType = null as any;
				}
			}
			if (approvalType) {
	...
	private async _triggerAutoCommit(toolName: string): Promise<void> {
		try {
			const workspaceFolders = this._workspaceContextService.getWorkspace().folders;
			const rootPath = workspaceFolders[0]?.uri.fsPath;
			if (!rootPath) return;

			const isDirty = await this._generateCommitMessageService.isWorkspaceDirty(rootPath);
			if (isDirty) {
				const message = `AI applied tool ${toolName} to modify workspace`;
				await this._generateCommitMessageService.createAutoCommit(rootPath, message);
			}
		} catch (error) {
	...
	private _clearRagContextCacheForThread(threadId: string): void {
		const prefix = `${threadId}:`;
		for (const key of [...this._hybridRagContextCache.keys()]) {
			if (key.startsWith(prefix)) {
				this._hybridRagContextCache.delete(key);
			}
		}
	}
	...
	private async _runChatAgent({
		...
		this._clearRagContextCacheForThread(threadId)
		...
		while (shouldSendAnotherMessage) {
			...
			if (lastUserMsgIdx !== -1) {
				...
				if (lastUserMsg.role === 'user') {
					try {
						const queryText = lastUserMsg.displayContent || lastUserMsg.content
						if (queryText) {
							const cacheKey = `${threadId}:${lastUserMsgIdx}`
							let context = this._hybridRagContextCache.get(cacheKey)
							if (context === undefined) {
								const selections = lastUserMsg.selections?.length
									? lastUserMsg.selections
									: lastUserMsg.state.stagingSelections
								context = await this._gatherHybridRagContext(queryText, selections)
			...
			while (shouldRetryLLM) {
				...
				const finalContent = await this._validateAndSelfRepairDiagrams(
					threadId,
					info.fullText,
					modelSelection,
					modelSelectionOptions,
				...
				this._addMessageToThread(threadId, { role: 'assistant', displayContent: finalContent, reasoning: info.fullReasoning, anthropicReasoning: info.anthropicReasoning })
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._mcodeModelService.getModelFromFsPath(fsPath)
			if (!model) continue
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		const thread = this.state.allThreads[threadId]
		if (!thread) return
		const { model } = this._mcodeModelService.getModel(uri)
...
*/
	...
	private async _addUserMessageAndStreamResponse({ userMessage, _chatSelections, threadId }: { userMessage: string, _chatSelections?: StagingSelectionItem[], threadId: string }) {
		...
		if (userMessage.trim() === '/undo') {
			...
			try {
				await this.performUndo(threadId);
			} catch (e) {
				// Error is already handled/notified in performUndo
			}
			return;
		}
		...
		this._wrapRunAgentToNotify(
			this._runChatAgent({ threadId, ...this._currentModelSelectionProps(instructions), }),
			threadId,
		)
	...
	async performUndo(threadId: string): Promise<void> {
		try {
			const workspaceFolders = this._workspaceContextService.getWorkspace().folders;
			const rootPath = workspaceFolders[0]?.uri.fsPath;
			if (!rootPath) {
				this._notificationService.warn('No active workspace found to perform undo.');
				return;
			}
			await this._generateCommitMessageService.performUndo(rootPath);
			this._addMessageToThread(threadId, {
				role: 'assistant',
				displayContent: '🤖 **Undo successful!** The latest AI changes have been rolled back, and the workspace was reverted to the previous version.',
				reasoning: '',
				anthropicReasoning: null
			});
			this._notificationService.info('Undo successful. Workspace reverted.');
		} catch (error: any) {
			console.error(error);
			this._notificationService.error(`Undo failed: ${error.message}`);
			throw error;
		}
	}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._mcodeModelService.getModelSafe(uri)
	...
	deleteThread(threadId: string): void {
		...
		if (currentThreadId === threadId) {
			...
			if (remainingIds.length > 0) {
				// Sort remaining threads by lastModified descending to pick the most recent one
				remainingIds.sort((a, b) => {
					const timeA = new Date(newThreads[a]!.lastModified).getTime();
					const timeB = new Date(newThreads[b]!.lastModified).getTime();
					return timeB - timeA;
				});
				const nextCurrentThreadId = remainingIds[0];
				this._setState({ allThreads: newThreads, currentThreadId: nextCurrentThreadId });
			} else {
				// No threads left, create a new one
				const newThread = newThreadObject();
				const finalThreads = { [newThread.id]: newThread };
				this._storeAllThreads(finalThreads);
				this._setState({ allThreads: finalThreads, currentThreadId: newThread.id });
			}
		} else {
			this._setState({ allThreads: newThreads });
		}
	...
	private async _refreshLspSnippets(): Promise<string[]> {
		...
		try {
			await this._contextGatheringService.updateCache(model, pos)
		} catch (e) {
			console.warn('[RAG] LSP cache update failed:', e)
		}
		return this._contextGatheringService.getCachedSnippets()
	}

	private async _gatherHybridRagContext(
		queryText: string,
		stagingSelections?: StagingSelectionItem[] | null,
	): Promise<string | null> {
		const gs = this._settingsService.state.globalSettings
		const contextMax = gs.ragContextMaxChars ?? 12_000
		const lspRatio = gs.ragLspBudgetRatio ?? 0.3
		const ragOptions = {
			...defaultRagQueryOptions,
			similarityTopK: gs.ragSimilarityTopK ?? defaultRagQueryOptions.similarityTopK,
			finalTopK: gs.ragFinalTopK ?? defaultRagQueryOptions.finalTopK,
			useReranker: gs.ragUseReranker ?? defaultRagQueryOptions.useReranker,
			useOrchestrator: gs.ragUseOrchestrator ?? defaultRagQueryOptions.useOrchestrator,
			useSubQuestions: gs.ragUseSubQuestions ?? defaultRagQueryOptions.useSubQuestions,
			useLlmSubQuestions: gs.ragUseLlmSubQuestions ?? defaultRagQueryOptions.useLlmSubQuestions,
			useRouter: gs.ragUseRouter ?? defaultRagQueryOptions.useRouter,
			useGraphExpand: gs.ragUseGraphExpand ?? defaultRagQueryOptions.useGraphExpand,
			graphExpandHops: gs.ragGraphExpandHops ?? defaultRagQueryOptions.graphExpandHops,
			graphExpandMax: gs.ragGraphExpandMax ?? defaultRagQueryOptions.graphExpandMax,
			useDocLinkedCode: gs.ragUseDocLinkedCode ?? defaultRagQueryOptions.useDocLinkedCode,
			assemblyMaxChars: Math.floor(contextMax * (1 - lspRatio)),
			ragCompactMode: gs.ragCompactMode ?? false,
			ragIntentOrchestration: gs.ragIntentOrchestration ?? true,
		}
		...
		const { merged, hasLsp, hasVector } = mergeRagContexts({
			...
			maxTotalChars: gs.ragContextMaxChars ?? 12_000,
			lspBudgetRatio: gs.ragLspBudgetRatio ?? 0.3,
			gitMaxChars: gs.ragGitContextMaxChars ?? 2_000,
			excludeFilePaths,
		})
	...
	private async _validateAndSelfRepairDiagrams(
		...
		const drawioRegex = /```drawio\n([\s\S]*?)\n```/g;
		const manimRegex = /```manim\n([\s\S]*?)\n```/g;

		let hasError = false;
		let errorType: 'mermaid' | 'drawio' | 'manim' | null = null;
		let errorCode = '';
		let errorMessage = '';

		// 1. Mermaid validation
		const mermaidMatches = [...assistantContent.matchAll(mermaidRegex)];
		for (const m of mermaidMatches) {
			try {
				const mermaid = (await import('mermaid')).default;
				mermaid.initialize({ startOnLoad: false, theme: 'dark', securityLevel: 'loose' });
				await mermaid.parse(m[1].trim());
			} catch (err: any) {
				hasError = true;
				errorType = 'mermaid';
				errorCode = m[1];
				errorMessage = err.message || 'Mermaid parsing failed';
				break;
			}
		}
		...
		// 2. Drawio validation
		if (!hasError) {
			const drawioMatches = [...assistantContent.matchAll(drawioRegex)];
			for (const m of drawioMatches) {
				try {
					const parser = new DOMParser();
					const doc = parser.parseFromString(m[1].trim(), 'text/xml');
					...
					if (!hasGraphModel) {
						...
						errorType = 'drawio';
						errorCode = m[1];
						errorMessage = 'XML is missing required <mxGraphModel> root element.';
						break;
					}
				} catch (err: any) {
					hasError = true;
					errorType = 'drawio';
					errorCode = m[1];
					errorMessage = err.message || 'Draw.io XML parsing failed';
					break;
				}
			}
		}
		...
		// 3. Manim validation
		if (!hasError) {
			const manimMatches = [...assistantContent.matchAll(manimRegex)];
			for (const m of manimMatches) {
				const workspaceFolders = this._workspaceContextService.getWorkspace().folders;
				const rootPath = workspaceFolders[0]?.uri.fsPath;
				if (rootPath) {
					const res = await this._mcodeDiagramService.renderManim(m[1].trim(), rootPath);
					if (!res.success) {
						hasError = true;
						errorType = 'manim';
						errorCode = m[1];
						errorMessage = res.error || 'Manim execution failed';
						break;
					}
				}
			}
		...
		// If error is found and retries remaining, run LLM correction
		if (hasError && retryCount < 2) {
			console.log(`[Self-Repair] Found error in ${errorType} diagram. Retrying (${retryCount + 1}/2)...`);
			const correctionPrompt = `你刚刚生成的 ${errorType} 图表代码存在语法错误，解析器或运行环境报错。
报错信息如下：
${errorMessage}

请严格遵守 ${errorType} 语法规范，修复上述错误，并重新输出完整的 ${errorType} 代码块。请不要包含任何解释性文字。
错误代码：
\`\`\`${errorType}
${errorCode}
\`\`\`
`;
			...
			const currentThreadMessages = this.state.allThreads[threadId]?.messages ?? [];
			const tempChatMessages = [
				...
				},
				{
					...
					displayContent: correctionPrompt,
					selections: [],
					state: defaultMessageState
				}
			];

			const { messages: llmMessages, separateSystemMessage: tempSysMsg } = await this._convertToLLMMessagesService.prepareLLMChatMessages({
				chatMessages: tempChatMessages,
				modelSelection,
				chatMode
			});
			const repairPromise = new Promise<string>((resolve, reject) => {
				const cancelToken = this._llmMessageService.sendLLMMessage({
					...
					modelSelectionOptions,
					overridesOfModel,
					logging: { loggingName: `Chat - Repair ${errorType}` },
					separateSystemMessage: tempSysMsg,
					onText: ({ fullText }) => {
						this._setStreamState(threadId, {
							isRunning: 'LLM',
							llmInfo: { displayContentSoFar: `[Self-Repairing ${errorType}...]`, reasoningSoFar: '', toolCallSoFar: null },
							interrupt: Promise.resolve(() => { if (cancelToken) this._llmMessageService.abort(cancelToken); })
						});
					},
			...
			try {
				const rawRepaired = await repairPromise;
				let cleanRepaired = rawRepaired.trim();
				const cleanRepairedBlockMatch = cleanRepaired.match(new RegExp(`\`\`\`${errorType}\\n([\\s\\S]*?)\\n\`\`\``));
				if (cleanRepairedBlockMatch) {
					cleanRepaired = cleanRepairedBlockMatch[0];
				} else if (!cleanRepaired.startsWith('```')) {
					cleanRepaired = `\`\`\`${errorType}\n${cleanRepaired}\n\`\`\``;
				}

				const invalidBlock = `\`\`\`${errorType}\n${errorCode}\n\`\`\``;
				const updatedContent = assistantContent.replace(invalidBlock, cleanRepaired);

				return this._validateAndSelfRepairDiagrams(
					threadId,
					updatedContent,
					modelSelection,
					modelSelectionOptions,
				...
				return assistantContent + `\n\n> ⚠️ **${errorType} Diagram Validation Failed**:\n> \`\`\`\n> ${errorMessage}\n> \`\`\``;
			}
		} else if (hasError) {
			return assistantContent + `\n\n> ⚠️ **${errorType} Diagram Validation Failed (exceeded retries)**:\n> \`\`\`\n> ${errorMessage}\n> \`\`\``;
		}
