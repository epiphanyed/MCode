...
import { IVoidRagService, RagRelatedDependency } from '../common/mcodeRagTypes.js';
...
export interface IContextGatheringService {
	...
	getRelatedDependencies(fileUri: URI, maxResults?: number): Promise<RagRelatedDependency[]>;
}
...
class ContextGatheringService extends Disposable implements IContextGatheringService {
	...
	constructor(
		...
		@IVoidRagService private readonly _ragService: IVoidRagService,
	) {
		super();
	...
	public async getRelatedDependencies(fileUri: URI, maxResults = 8): Promise<RagRelatedDependency[]> {
		const fsPath = fileUri.fsPath;
		try {
			const fromGraph = await this._ragService.getRelatedDependencies(fsPath, maxResults);
			if (fromGraph.length > 0) {
				return fromGraph;
			}
		} catch (err) {
			console.warn('[ContextGathering] Code graph dependencies unavailable:', err);
		}
		return this._getRelatedDependenciesViaLsp(fileUri, maxResults);
	}

	private async _getRelatedDependenciesViaLsp(fileUri: URI, maxResults: number): Promise<RagRelatedDependency[]> {
		const model = this._modelService.getModel(fileUri);
		if (!model) {
			return [];
		}

		const deps: RagRelatedDependency[] = [];
		const seen = new Set<string>();
		const selfPath = fileUri.fsPath;

		const addRef = (uri: URI, reason: string) => {
			const p = uri.fsPath;
			if (p === selfPath || seen.has(p)) {
				return;
			}
			seen.add(p);
			deps.push({ filePath: p, kind: 'references', reason });
		};

		const symbols = await this._getSymbolsInRange(model, model.getFullModelRange());
		const topSymbols = symbols
			.filter(s => s.kind === SymbolKind.Function || s.kind === SymbolKind.Method || s.kind === SymbolKind.Class)
			.slice(0, 5);

		const refProviders = this._langFeaturesService.referenceProvider.ordered(model);
		for (const sym of topSymbols) {
			if (deps.length >= maxResults) {
				break;
			}
			const pos = new Position(sym.range.startLineNumber, sym.range.startColumn);
			for (const provider of refProviders) {
				try {
					const refs = await provider.provideReferences(model, pos, { includeDeclaration: false }, CancellationToken.None);
					if (!refs) {
						continue;
					}
					for (const ref of refs) {
						addRef(ref.uri, `references ${sym.name}`);
						if (deps.length >= maxResults) {
							break;
						}
					}
