...
import { mountMcodeCommandBar } from './react/out/mcode-editor-widgets-tsx/index.js'
...
export interface IMcodeCommandBarService {
...
export const IMcodeCommandBarService = createDecorator<IMcodeCommandBarService>('McodeCommandBarService');
...
export class McodeCommandBarService extends Disposable implements IMcodeCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.McodeCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._mcodeModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IMcodeCommandBarService, McodeCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type McodeCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountMcodeCommandBar(root, accessor, { uri, editor } satisfies McodeCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies McodeCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptDiffAction', 'MCode: Accept Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectDiffAction', 'MCode: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToNextDiffAction', 'MCode: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToPrevDiffAction', 'MCode: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToNextUriAction', 'MCode: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidGoToPrevUriAction', 'MCode: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptFileAction', 'MCode: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectFileAction', 'MCode: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidAcceptAllDiffsAction', 'MCode: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			f1: true,
			title: localize2('voidRejectAllDiffsAction', 'MCode: Reject All Diffs in All Files'),
			keybinding: {
