...
import { mountMcodeOnboarding } from './react/out/mcode-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.mcodeOnboarding';
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.mcode-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountMcodeOnboarding(onboardingContainer, accessor);
