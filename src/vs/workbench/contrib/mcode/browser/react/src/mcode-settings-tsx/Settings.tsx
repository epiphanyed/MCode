	...
	return <div className=''>
		{modelDump.map((m, i) => {
			...
			const detailAboutModel = type === 'autodetected' ?
				<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='mcode-tooltip' data-tooltip-place='right' data-tooltip-content='Detected locally' />
				: type === 'custom' ?
					<Asterisk size={14} className="inline-block align-text-top brightness-115 stroke-[2] text-[#0e70c0]" data-tooltip-id='mcode-tooltip' data-tooltip-place='right' data-tooltip-content='Custom model' />
					: undefined
...
const ModelRouterPicker = ({
	...
	label: string;
	providerSetting: GlobalSettingName;
	modelSetting: GlobalSettingName;
}) => {
	const accessor = useAccessor();
	const mcodeSettingsService = accessor.get('IVoidSettingsService');
	const settingsState = useSettingsState();
	...
	return (
		<div className='flex flex-col gap-1'>
			<span className='text-xs text-void-fg-3'>{label}</span>
			<VoidCustomDropdownBox
				options={options}
				...
				className='text-xs text-void-fg-3 bg-void-bg-1 border border-void-border-1 rounded p-0.5 px-1 w-full'
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: { sayWeAutoDetect?: boolean }) => {
	return <div className='prose-p:my-0 prose-ol:list-decimal prose-p:py-0 prose-ol:my-0 prose-ol:py-0 prose-span:my-0 prose-span:py-0 text-void-fg-3 text-sm list-decimal select-text'>
		...
		{sayWeAutoDetect && <div className=' pl-6'><ChatMarkdownRender string={`MCode automatically detects locally running models and enables them.`} chatMessageLocation={undefined} /></div>}
	</div>
}
...
const IndexActionFeedbackView = ({ feedback }: { feedback: IndexActionFeedback }) => {
	...
	if (feedback.kind === 'info') {
		return (
			<span className="text-[#0e70c0] text-sm flex items-center gap-1">
	...
	return <span className="text-red-500 text-sm">{feedback.message}</span>
}
...
const IndexSettingsSection = () => {
	...
	return (
		<div className="flex flex-col gap-6 my-4 max-w-[600px]">
			{/* Active Status Banner */}
			<div className="flex items-center justify-between p-3.5 rounded-lg border border-void-border-2 bg-void-bg-2-alt/40">
				<div className="flex items-center gap-2.5">
					<div className={`size-2.5 rounded-full ${activeIndexType ? 'bg-green-500 animate-pulse' : 'bg-yellow-500 animate-pulse'}`}></div>
					<span className="text-sm text-void-fg-3">
						Currently Active Index:
					</span>
					{activeIndexType ? (
						<span className="text-sm font-semibold text-void-fg-1">
							{activeIndexType === 'milvus' ? 'Milvus (Vector Database)' : 'Local Index (Disk)'}
						</span>
					) : (
						<span className="text-sm text-void-fg-3 italic">Initializing...</span>
					)}
				</div>
				{settingsState.globalSettings.indexType !== activeIndexType && activeIndexType !== null && (
					<div className="text-[11px] text-yellow-500 px-2 py-0.5 rounded border border-yellow-500/20 bg-yellow-500/5">
			...
			<div className="flex flex-col gap-2">
				<span className="text-void-fg-1 text-sm font-medium">Index Type</span>
					...
					<button
						...
						className={`px-4 py-2 rounded-md border transition-all duration-200 ${
							settingsState.globalSettings.indexType === 'local'
								...
								: 'bg-void-bg-2 border-void-border-2 text-void-fg-3 hover:bg-void-bg-2/80'
						} ${isIndexRunning ? 'opacity-50 cursor-not-allowed' : ''}`}
					>
						Local Index
					</button>
					<button
			...
			<div className="flex flex-col gap-4 p-4 rounded-md border border-void-border-2 bg-void-bg-2-alt">
				<h4 className="text-md font-medium text-void-fg-1 border-b border-void-border-2 pb-2">Embedding Model Configuration</h4>
				
				<div className="flex flex-col gap-2">
					<label className="text-xs text-void-fg-3">Embedding Provider</label>
					<div className="flex gap-2">
						<button
							onClick={() => {
				...
				<div className="flex flex-col gap-1.5">
					<label className="text-xs text-void-fg-3">Embedding Model Name</label>
					<VoidSimpleInputBox
						...
						compact={true}
					/>
					<span className="text-[11px] text-void-fg-3 italic">
						{embeddingProvider === 'ollama'
				...
				{embeddingProvider === 'ollama' && (
					<div className="flex flex-col gap-1.5">
						<label className="text-xs text-void-fg-3">Ollama Endpoint</label>
						<VoidSimpleInputBox
			...
			{settingsState.globalSettings.indexType === 'local' && (
				<div className="flex flex-col gap-4 p-4 rounded-md border border-void-border-2 bg-void-bg-2-alt">
					<h4 className="text-md font-medium text-void-fg-1 border-b border-void-border-2 pb-2">Local Index Status</h4>

					<div className="flex flex-col gap-2 text-sm">
						<div className="flex items-center gap-2">
							<div className={`size-2 rounded-full ${
								ragIndexState.error ? 'bg-red-500' :
								isIndexRunning ? 'bg-blue-500 animate-pulse' :
								ragIndexState.fileCount > 0 ? 'bg-green-500' : 'bg-yellow-500'
							}`} />
							<span className="text-void-fg-1 font-medium">{ragPhaseLabel(ragIndexState.phase)}</span>
						</div>
						<div className="grid grid-cols-2 gap-x-4 gap-y-1 text-void-fg-3 text-xs">
							<span>Files indexed: <span className="text-void-fg-2">{isIndexRunning ? ragIndexState.filesDone : ragIndexState.fileCount}</span></span>
							<span>Chunks: <span className="text-void-fg-2">{ragIndexState.chunkCount}</span></span>
							<span className="col-span-2">Last built: <span className="text-void-fg-2">{formatBuiltAt(ragIndexState.builtAt)}</span></span>
						</div>
						{isIndexRunning && progressPercent !== null && (
							<div className="flex flex-col gap-1 mt-1">
								<div className="h-1.5 w-full rounded-full bg-void-bg-2 overflow-hidden">
									<div
						...
						{isIndexRunning && progressPercent === null && ragIndexState.phase === 'persisting' && (
							<div className="flex items-center gap-2 text-[11px] text-void-fg-3 mt-1">
						...
						{ragIndexState.error && (
							<span className="text-red-500 text-xs">{ragIndexState.error}</span>
						)}
					</div>

					<div className="flex flex-col gap-3 border-t border-void-border-2 pt-4">
						<h5 className="text-sm font-medium text-void-fg-1">Retrieval Settings</h5>
						<p className="text-[11px] text-void-fg-3">
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">Retrieve TopK</label>
								<VoidSimpleInputBox
							...
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">Final TopK</label>
								<VoidSimpleInputBox
									value={ragFinalTopK}
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Hybrid reranker (vector 70% + keyword 30%)</span>
							<VoidSwitch
						...
						<div className="grid grid-cols-3 gap-3">
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">Context max chars</label>
								<VoidSimpleInputBox
							...
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">LSP budget ratio (0.1–0.8)</label>
								<VoidSimpleInputBox
							...
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">Git context max chars</label>
								<VoidSimpleInputBox
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Compact RAG code chunks</span>
							<VoidSwitch
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Intent-lite orchestration (simple queries)</span>
							<VoidSwitch
					...
					<div className="flex flex-col gap-3 border-t border-void-border-2 pt-4">
						<h5 className="text-sm font-medium text-void-fg-1">Orchestration</h5>
						<p className="text-[11px] text-void-fg-3">
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Enable orchestrator</span>
							<VoidSwitch
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Router (code / git / doc)</span>
							<VoidSwitch
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Sub-question split</span>
							<VoidSwitch
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">LLM sub-question split (needs OpenAI key)</span>
							<VoidSwitch
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Graph expansion</span>
							<VoidSwitch
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">Graph hops (1–2)</label>
								<VoidSimpleInputBox
							...
							<div className="flex flex-col gap-1">
								<label className="text-xs text-void-fg-3">Graph max snippets</label>
								<VoidSimpleInputBox
						...
						<div className="flex items-center justify-between">
							<span className="text-xs text-void-fg-3">Doc → linked code</span>
							<VoidSwitch
			...
			{settingsState.globalSettings.indexType === 'milvus' && (
				<div className="flex flex-col gap-4 p-4 rounded-md border border-void-border-2 bg-void-bg-2-alt">
					<h4 className="text-md font-medium text-void-fg-1 border-b border-void-border-2 pb-2">Milvus Connection Details</h4>

					<div className="flex flex-col gap-1.5">
						<label className="text-xs text-void-fg-3">Milvus Address / URL</label>
						<VoidSimpleInputBox
							value={milvusUrl}
					...
					<div className="flex flex-col gap-1.5">
						<label className="text-xs text-void-fg-3">Username (optional)</label>
						<VoidSimpleInputBox
					...
					<div className="flex flex-col gap-1.5">
						<label className="text-xs text-void-fg-3">Password (optional)</label>
						<VoidSimpleInputBox
					...
					<div className="flex items-center gap-2">
						...
						/>
						<label htmlFor="rag-milvus-dual-write" className="text-xs text-void-fg-3">
							Dual-write: also keep local disk index (migration / backup)
						</label>
					</div>

					<div className="flex items-center gap-4 mt-4 border-t border-void-border-2 pt-4 flex-wrap">
						<VoidButtonBgDarken
							onClick={handleTestMilvusConnection}
							disabled={isIndexRunning || !milvusUrl}
							className="px-4 py-2 bg-void-bg-2 border border-void-border-2 text-void-fg-1"
						>
							Test Connection
						</VoidButtonBgDarken>
						<VoidButtonBgDarken
						...
						{milvusTestMessage && (
							<span className={`text-sm ${milvusTestMessage.startsWith('Connected') ? 'text-green-500' : 'text-void-fg-3'}`}>
								{milvusTestMessage}
							</span>
						)}
					</div>
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`MCode's Settings`}</h1>
						...
						<div className='flex flex-col gap-12'>
							...
							<div className={shouldShowTab('indexSettings') ? `` : 'hidden'}>
								<ErrorBoundary>
									<h2 className={`text-3xl mb-2`}>Index Settings</h2>
									<h3 className={`text-void-fg-3 mb-4`}>
							...
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`MCode can access any model that you host locally. We automatically detect your local models by default.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : 'hidden'}>
								<ErrorBoundary>
									...
									<h3 className={`text-void-fg-3 mb-2`}>{`MCode can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
								...
								<ErrorBoundary>
									...
									<div className='flex flex-col gap-y-8 my-4'>
										...
										<div className='w-full'>
											...
											<div className='text-sm text-void-fg-3 mt-1'>{`Settings that control the visibility of MCode suggestions in the code editor.`}</div>
							...
							<div className={`${shouldShowTab('general') ? `` : 'hidden'} flex flex-col gap-12`}>
								...
								<div>
									<ErrorBoundary>
										<h2 className='text-3xl mb-2'>Model Intent Routing</h2>
										...
										<div className='flex flex-col gap-3 max-w-md'>
											<div className='flex items-center gap-x-2'>
												...
												/>
												<span className='text-void-fg-3 text-xs'>Enable intent-based model routing</span>
											</div>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className='text-void-fg-3 mb-4'>{`Transfer your editor settings into MCode.`}</h4>
								...
								<div>
									...
									<h4 className='text-void-fg-3 mb-4'>{`Transfer MCode's settings and chats in and out of MCode.`}</h4>
									<div className='flex flex-col gap-8'>
										...
										<div className='flex flex-col gap-2 max-w-48 w-full'>
											...
											<ConfirmButton className='px-4 py-1 w-full' onConfirm={() => { mcodeSettingsService.resetState(); }}>
												Reset Settings
											</ConfirmButton>
										</div>
								...
								<div className='max-w-[600px]'>
									...
									<h4 className={`text-void-fg-3 mb-4`}>Very basic anonymous usage tracking helps us keep MCode running smoothly. You may opt out below. Regardless of this setting, MCode never sees your code, messages, or API keys.</h4>
