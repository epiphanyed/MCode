...
const ContextUsageRing: React.FC<{ featureName: FeatureName }> = ({ featureName }) => {
	...
	return (
		<div
			className="flex items-center gap-1.5 px-1 py-0.5 select-none pointer-events-auto rounded text-[10px] text-void-fg-3 hover:text-void-fg-1 transition-colors"
		...
		>
			<div className="relative flex items-center justify-center w-5 h-5">
				...
				<span className="absolute text-[8px] font-medium mt-[0.5px]">
					{percentage}%
				</span>
			</div>
					...
					>
						...
						>
							...
							{selection.type === 'File' && type === 'staging' && !isThisSelectionProspective ?
								<span
									className='text-[8px] text-void-fg-3 cursor-pointer hover:text-void-fg-1 z-1'
...
const PtyTerminalConsole = ({ terminalId, threadId }: { terminalId: string, threadId: string }) => {
	...
	return (
		<div className="flex flex-col border border-void-border-3 rounded bg-void-bg-1 overflow-hidden h-[300px] w-full">
			<div className="flex-1 overflow-auto p-2 font-mono text-xs text-void-fg-2 whitespace-pre-wrap select-text">
				{removeAnsiEscapeCodes(logs) || 'Starting command execution...'}
				<div ref={logsEndRef} />
			</div>
			{isAwaitingInput && (
				<div className="p-2 border-t border-void-border-3 bg-void-bg-2 flex items-center gap-2">
					<span className="text-xs text-yellow-500 font-semibold animate-pulse">Waiting: {promptText}</span>
					<input
						type="text"
						className="flex-1 text-xs bg-void-bg-1 border border-void-border-3 rounded px-1.5 py-0.5 text-void-fg-1 focus:outline-none focus:border-void-primary"
						value={inputValue}
					...
					/>
					<button
						className="text-xs bg-void-primary text-white px-2 py-0.5 rounded hover:brightness-110"
