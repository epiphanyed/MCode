...
import { MAX_CHILDREN_URIs_PAGE, MAX_DIRSTR_CHARS_TOTAL_BEGINNING, MAX_DIRSTR_CHARS_SYSTEM, MAX_DIRSTR_CHARS_TOTAL_TOOL } from './prompt/prompts.js';
...
const SYSTEM_MAX_DEPTH = 1;
const SYSTEM_MAX_ITEMS_PER_DIR = 40;
...
export interface IDirectoryStrService {
	...
	/** Compact workspace tree for system prompt (CTX-B3). */
	getSystemDirectoriesStr(opts: { cutOffMessage: string }): Promise<string>
...
class DirectoryStrService extends Disposable implements IDirectoryStrService {
	...
	async getSystemDirectoriesStr({ cutOffMessage }: { cutOffMessage: string }): Promise<string> {
		let str = '';
		let cutOff = false;
		const folders = this.workspaceContextService.getWorkspace().folders;
		if (folders.length === 0) {
		...
		for (let i = 0; i < folders.length; i += 1) {
			...
			const f = folders[i];
			str += `Directory of ${f.uri.fsPath}:\n`;
			const eRoot = await this.fileService.resolve(f.uri);
			if (!eRoot) {
				continue;
			}
			const { content, wasCutOff } = await computeAndStringifyDirectoryTree(
				eRoot,
				this.fileService,
				MAX_DIRSTR_CHARS_SYSTEM - str.length,
				{ count: 0 },
				{ maxDepth: SYSTEM_MAX_DEPTH, currentDepth: 0, maxItemsPerDir: SYSTEM_MAX_ITEMS_PER_DIR },
			);
			str += content;
			if (wasCutOff) {
				cutOff = true;
