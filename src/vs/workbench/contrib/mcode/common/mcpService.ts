...
import { MCPUserStateOfName } from './mcodeSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	private async _initialize() {
		try {
			await this.mcodeSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.mcodeSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.mcodeSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.mcodeSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.mcodeSettingsService.setMCPServerState(serverName, { isOn });
