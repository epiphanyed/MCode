...
import { ChatMode } from '../../common/mcodeSettingsTypes.js'
...
const parseXMLPrefixToToolCall = <T extends ToolName,>(toolName: T, toolId: string, str: string, toolOfToolName: ToolOfToolName): RawToolCallObj => {
	...
	while (true) {
		...
		if (matchedOpenParam === null) {
			// Do not append trailing irrelevant text/tags (like <timeout> or model chatter)
			// to the previously closed valid parameter.
			return getAnswer()
		}
		else {
