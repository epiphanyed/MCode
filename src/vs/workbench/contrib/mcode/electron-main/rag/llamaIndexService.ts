...
export class LlamaIndexService {
    ...
    private pathInSkippedDir(filePath: string): boolean {
        ...
        return pathContainsSkippedDirectory(relativePath);
    }
    ...
    private logIndexFileFailureSummary(totalFiles: number): void {
        if (this.indexFileFailureCount === 0) {
            return;
        }
        console.warn(
            `[RAG] Index build: ${this.indexFileFailureCount}/${totalFiles} file(s) skipped due to read/chunk errors. Samples: ${this.indexFileFailureSamples.join(', ')}`,
        );
    }

    private async updateCodeGraphSafely(normalizedPath: string, content: string): Promise<void> {
        try {
            await this.updateCodeGraphForFile(normalizedPath, content);
        } catch (err) {
            console.warn(`[RAG] Code graph update failed for ${normalizedPath} (indexed chunks kept):`, err);
        }
    }
    ...
    private walkDir(dir: string, fileList: string[] = []): string[] {
        ...
        for (const file of files) {
            ...
            if (stat.isDirectory()) {
                if (shouldSkipDirectoryName(file)) {
                    continue;
                }
    ...
    private async scanWorkspaceNodesCore(): Promise<ScanIndexResult> {
        ...
        for (let i = 0; i < targetFiles.length; i++) {
            ...
            try {
                ...
                const stage = e instanceof Error && e.message.includes('ENOENT') ? 'read' : 'chunk';
                this.recordIndexFileFailure(filePath, stage, e);
            }
        ...
        this.logIndexFileFailureSummary(targetFiles.length);
