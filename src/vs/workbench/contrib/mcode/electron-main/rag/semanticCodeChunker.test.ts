...
`;
	...
	test('Scilab nested functions and false positive endfunction keywords', () => {
		const content = `function y = outer(x)
  function z = inner(a)
    z = a + 1
  endfunction // end of inner
  disp("Do not match endfunction inside string literals!")
  y = inner(x) * 2
endfunction
`;
		const chunks = chunkCodeSemantically(content, 'nested.sci');
		assert.strictEqual(chunks.length, 1);
		assert.strictEqual(chunks[0].symbolName, 'outer');
		assert.strictEqual(chunks[0].startLine, 1);
		assert.strictEqual(chunks[0].endLine, 7);
	});
