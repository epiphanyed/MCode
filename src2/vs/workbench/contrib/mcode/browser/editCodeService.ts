...
import { FeatureName } from '../common/mcodeSettingsTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._mcodeModelService.initializeModel(model.uri)
	...
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/voideditor/mcode/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const onFinishEdit = async () => {
			...
			await this._mcodeModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._mcodeModelService.initializeModel(uri)
		await this._mcodeModelService.saveModel(uri) // save the URI
	}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`MCode: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`MCode error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IMcodeCommandBarService private readonly _mcodeCommandBarService: IMcodeCommandBarService,
		...
		const getAcceptRejectText = () => {
			...
			const commandBarStateAtUri = this._mcodeCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._mcodeCommandBarService.onDidChangeState(e => {
