...
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/mcodeSettingsTypes.js'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	async generateCommitMessage() {
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const modelSelection = this.mcodeSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.mcodeSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.mcodeSettingsService.state.overridesOfModel
	...
	async isWorkspaceDirty(path: string): Promise<boolean> {
		return this.voidSCM.isWorkspaceDirty(path);
	}

	async createAutoCommit(path: string, message: string): Promise<void> {
		return this.voidSCM.createAutoCommit(path, message);
	}

	async performUndo(path: string): Promise<void> {
		return this.voidSCM.performUndo(path);
	}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCommitMessagePrompt', 'MCode: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('voidCommitMessagePromptTooltip', 'MCode: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCommitMessagePromptCancel', 'MCode: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('voidCommitMessagePromptCancelTooltip', 'MCode: Cancel Commit Message Generation'),
