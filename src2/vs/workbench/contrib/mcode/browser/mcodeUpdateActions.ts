...
import { IVoidUpdateService } from '../common/mcodeUpdateService.js';
import { IWorkbenchContribution, registerWorkbenchContribution2, WorkbenchPhase } from '../../../common/contributions.js';
import * as dom from '../../../../base/browser/dom.js';
import { IUpdateService } from '../../../../platform/update/common/update.js';
import { VoidCheckUpdateRespose } from '../common/mcodeUpdateServiceTypes.js';
...
const notifyUpdate = (res: VoidCheckUpdateRespose & { message: string }, notifService: INotificationService, updateService: IUpdateService): INotificationHandle => {
	const message = res?.message || 'This is a very old version of MCode, please download the latest version from [GitHub](https://github.com/mcodeeditor/mcode/releases)!'
	...
	if (res?.action) {
		...
		if (res.action === 'reinstall') {
			primary.push({
				...
				run: () => {
					...
					window.open('https://github.com/mcodeeditor/mcode/releases')
				}
			})
		}
		...
		primary.push({
			...
			run: () => {
				const { window } = dom.getActiveWindow()
				window.open('https://github.com/mcodeeditor/mcode')
			}
		})
...
const notifyErrChecking = (notifService: INotificationService): INotificationHandle => {
	const message = `MCode Error: There was an error checking for updates. If this persists, please get in touch or download MCode [here](https://github.com/mcodeeditor/mcode/releases)!`
	const notifController = notifService.notify({
		severity: Severity.Info,
		message: message,
		sticky: true,
...
const performVoidCheck = async (
	...
	mcodeUpdateService: IVoidUpdateService,
	...
	const res = await mcodeUpdateService.check(explicit)
	if (!res) {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('voidCheckUpdate', 'MCode: Check for Updates'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const mcodeUpdateService = accessor.get(IVoidUpdateService)
		...
		const newController = await performVoidCheck(true, notifService, mcodeUpdateService, metricsService, updateService)
...
class VoidUpdateWorkbenchContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		@IVoidUpdateService mcodeUpdateService: IVoidUpdateService,
		...
		const autoCheck = () => {
			performVoidCheck(false, notifService, mcodeUpdateService, metricsService, updateService)
		}
