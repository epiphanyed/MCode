...
import { isFeatureNameDisabled } from '../../../../common/mcodeSettingsTypes.js';
...
export const useApplyStreamState = ({ applyBoxId }: {applyBoxId: string;}) => {
  const accessor = useAccessor();
  const mcodeCommandBarService = accessor.get('IMcodeCommandBarService');

  const getStreamState = useCallback(() => {
    const uri = getUriBeingApplied(applyBoxId);
    if (!uri) return 'idle-no-changes';
    return mcodeCommandBarService.getStreamState(uri);
  }, [mcodeCommandBarService, applyBoxId]);
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: {tooltipName: string;color?: IndicatorColor;position?: PlacesType;offset?: number;}) => ({
  'data-tooltip-id': color === 'orange' ? `mcode-tooltip-orange` : color === 'green' ? 'mcode-tooltip-green' : 'mcode-tooltip',
...
export const useEditToolStreamState = ({ applyBoxId, uri }: {applyBoxId: string;uri: URI;}) => {
  const accessor = useAccessor();
  const mcodeCommandBarService = accessor.get('IMcodeCommandBarService');
  const [streamState, setStreamState] = useState(mcodeCommandBarService.getStreamState(uri));
  // listen for stream updates on this box
  useCommandBarURIListener(useCallback((uri_) => {
    const shouldUpdate = uri.fsPath === uri_.fsPath;
    if (shouldUpdate) {setStreamState(mcodeCommandBarService.getStreamState(uri));}
  }, [mcodeCommandBarService, applyBoxId, uri]));
...
const ApplyButtonsForEdit = ({
  ...
  const onClickSubmit = useCallback(async () => {
    ...
    if (!applyDonePromise) {
      notificationService.info(`MCode Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`);
    }
    ...
    applyDonePromise?.catch((e) => {
      ...
      notificationService.info(`MCode Error: There was a problem running Apply: ${e}.`);
