...
import { ModelDropdown } from '../mcode-settings-tsx/ModelDropdown.js';
import { PastThreadsList } from './SidebarThreadSelector.js';
import { VOID_CTRL_L_ACTION_ID } from '../../../actionIDs.js';
import { VOID_OPEN_SETTINGS_ACTION_ID } from '../../../mcodeSettingsPane.js';
import { ITerminalPtyService } from '../../../terminalPtyService.js';
import { ITerminalToolService } from '../../../terminalToolService.js';
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/mcode/common/mcodeSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../mcode-settings-tsx/WarningBox.js';
...
const ReasoningOptionSlider = ({ featureName }: {featureName: FeatureName;}) => {
  ...
  if (canTurnOffReasoning && !reasoningBudgetSlider) {// if it's just a on/off toggle without a power slider
    return <div className="void-flex void-items-center void-gap-x-2">
        ...
        onChange={(newVal) => {
          ...
          mcodeSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff });
        }} />
  ...
  if (reasoningBudgetSlider?.type === 'budget_slider') {// if it's a slider
    ...
    return <div className="void-flex void-items-center void-gap-x-2">
        ...
        onChange={(newVal) => {
          ...
          mcodeSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal });
        }} />
  ...
  if (reasoningBudgetSlider?.type === 'effort_slider') {
    ...
    return <div className="void-flex void-items-center void-gap-x-2">
        ...
        onChange={(newVal) => {
          const isOff = canTurnOffReasoning && newVal === valueIfOff;
          mcodeSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined });
        }} />
...
const ChatModeDropdown = ({ className }: {className: string;}) => {
  ...
  const onChangeOption = useCallback((newVal: ChatMode) => {
    mcodeSettingsService.setGlobalSetting('chatMode', newVal);
...
const ContextUsageRing: React.FC<{featureName: FeatureName;}> = ({ featureName }) => {
  const accessor = useAccessor();
  ...
  const [usage, setUsage] = useState({ usedTokens: 0, maxTokens: 4096, percentage: 0 });

  useEffect(() => {
    const currentThread = chatThreadsService.getCurrentThread();
    const modelSelection = settingsState.modelSelectionOfFeature[featureName];
    if (!currentThread || !modelSelection) {
      setUsage({ usedTokens: 0, maxTokens: 4096, percentage: 0 });
      return;
    }

    let isMounted = true;
    convertService.estimateTokenUsage({
  ...
  }, [chatThreadsState, settingsState, featureName, chatThreadsService, convertService]);
  ...
  const formattedUsed = usage.usedTokens > 1000 ? `${(usage.usedTokens / 1000).toFixed(1)}k` : usage.usedTokens;
  const formattedMax = usage.maxTokens > 1000 ? `${(usage.maxTokens / 1000).toFixed(0)}k` : usage.maxTokens;

  return (
    <div
      className="void-flex void-items-center void-gap-1.5 void-px-1 void-py-0.5 void-select-none void-pointer-events-auto void-rounded void-text-[10px] void-text-void-fg-3 hover:void-text-void-fg-1 void-transition-colors"
      title={`Context usage: ${usage.usedTokens} / ${usage.maxTokens} tokens (${percentage}%)`}>

			<div className="void-relative void-flex void-items-center void-justify-center void-w-5 void-h-5">
				<svg className="void-w-full void-h-full void-transform -void-rotate-90">
					<circle
            cx="10"
            cy="10"
            r={radius}
            style={{ stroke: 'var(--vscode-editorWidget-border, rgba(128, 128, 128, 0.2))' }}
            strokeWidth="2"
            fill="transparent" />
            ...
            cy="10"
            r={radius}
            className="void-transition-all void-duration-300"
            ...
            strokeLinecap="round" />
				...
				<span className="void-absolute void-text-[8px] void-font-medium void-mt-[0.5px]">
					{percentage}%
				</span>
			</div>
			<span className="void-opacity-80">
    ...
    <div
      ...
      <>
          ...
          showProspectiveSelections={showProspectiveSelections} />
					...
					<DependencyRecommendations selections={selections} setSelections={setSelections} />
				</>
      }
        ...
        <div className="void-flex void-flex-col void-gap-y-1">
				...
				<div className="void-flex void-items-center void-gap-2">
					<ContextUsageRing featureName={featureName} />
        ...
        className={`void-flex void-flex-col void-space-y-[1px]`}>
          ...
          data-tooltip-delay-show={3000}>
              ...
              null
							...
							{selection.type === 'File' && type === 'staging' && !isThisSelectionProspective ?
              <span
                className="void-text-[8px] void-text-void-fg-3 void-cursor-pointer hover:void-text-void-fg-1 void-z-1"
                title={selection.state.includeFullFile ? 'Sending full file — click for summary' : 'Sending summary — click for full file'}
                onClick={(e) => {
                  e.stopPropagation();
                  ...
                  const newSelection: StagingSelectionItem = {
                    ...
                    state: {
                      ...selection.state,
                      includeFullFile: !selection.state.includeFullFile
                    }
                  };
                  setSelections([
                  ...selections.slice(0, i),
                  ...
                  ...selections.slice(i + 1)]
									...
									{selection.state.includeFullFile ? 'Full' : 'Summary'}
								</span> :
              null
              }
...
const toolNameToDesc = (toolName: BuiltinToolName, _toolParams: BuiltinToolCallParams[BuiltinToolName] | undefined, accessor: ReturnType<typeof useAccessor>): {
  ...
  const x = {
    ...
    'open_persistent_terminal': () => {
      ...
      return { desc1: toolParams.command ? `"${toolParams.command}"` : '' };
    },
...
const PtyTerminalConsole = ({ terminalId, threadId }: {terminalId: string;threadId: string;}) => {
  const accessor = useAccessor();
  const terminalPtyService = accessor.get('ITerminalPtyService');
  ...
  useEffect(() => {
    const targetId = terminalId || 'default-temp-terminal';

    const disposable = terminalToolsService.onPtyOutput((event) => {
      const eventId = event.terminalId || 'default-temp-terminal';
      const isMatched = eventId === targetId;
      if (isMatched) {
        setLogs((prev) => {
          const next = prev + event.data;
          if (next.length > 50000) return next.slice(-20000);
          return next;
        });
      }
    });

    const disposableInput = terminalToolsService.onPtyAwaitingInput((event) => {
      const eventId = event.terminalId || 'default-temp-terminal';
      if (eventId === targetId) {
  ...
  }, [terminalId, terminalToolsService]);
  ...
  const handleSendInput = () => {
    const ptyId = terminalToolsService.getPtyId(terminalId);
    if (ptyId && inputValue.trim().length > 0) {
      terminalPtyService.writePty(ptyId, inputValue + '\n');
  ...
  return (
    <div className="void-flex void-flex-col void-border void-border-void-border-3 void-rounded void-bg-void-bg-1 void-overflow-hidden void-h-[300px] void-w-full">
			<div className="void-flex-1 void-overflow-auto void-p-2 void-font-mono void-text-xs void-text-void-fg-2 void-whitespace-pre-wrap void-select-text">
				{removeAnsiEscapeCodes(logs) || 'Starting command execution...'}
				<div ref={logsEndRef} />
			</div>
			{isAwaitingInput &&
      <div className="void-p-2 void-border-t void-border-void-border-3 void-bg-void-bg-2 void-flex void-items-center void-gap-2">
					<span className="void-text-xs void-text-yellow-500 void-font-semibold void-animate-pulse">Waiting: {promptText}</span>
					<input
          type="text"
          className="void-flex-1 void-text-xs void-bg-void-bg-1 void-border void-border-void-border-3 void-rounded void-px-1.5 void-py-0.5 void-text-void-fg-1 focus:void-outline-none focus:void-border-void-primary"
          value={inputValue}
          ...
          autoFocus />
					...
					<button
          className="void-text-xs void-bg-void-primary void-text-white void-px-2 void-py-0.5 void-rounded hover:void-brightness-110"
...
const CommandTool = ({ toolMessage, type, threadId
  ...
  if (toolMessage.type === 'running_now') {
    if (type === 'run_command')
    componentParams.children = <PtyTerminalConsole terminalId={toolMessage.params.terminalId} threadId={threadId} />;
  } else
      ...
      if (toolMessage.type === 'success') {
        ...
        if (result.result) {
          ...
          <ToolChildrenWrapper className="void-whitespace-pre void-text-nowrap void-overflow-auto void-text-sm">
							<div className="!void-select-text void-cursor-auto">
								<BlockCode initValue={`${result.result.trim()}`} language='shellscript' />
							</div>
						</ToolChildrenWrapper>;
...
export const SidebarChat = () => {
  ...
  const initiallySuggestedPromptsHTML = <div className="void-flex void-flex-col void-gap-2 void-w-full void-text-nowrap void-text-void-fg-3 void-select-none">
    ...
    'Create a .mcoderules file for me'].
    map((text, index) =>
    <div
      key={index}
