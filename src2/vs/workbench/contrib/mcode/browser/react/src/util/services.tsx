...
import { MCPUserState, RefreshableProviderName, SettingsOfProvider } from '../../../../../../../workbench/contrib/mcode/common/mcodeSettingsTypes.js';
import { DisposableStore, IDisposable } from '../../../../../../../base/common/lifecycle.js';
import { VoidSettingsState } from '../../../../../../../workbench/contrib/mcode/common/mcodeSettingsService.js';
import { ColorScheme } from '../../../../../../../platform/theme/common/theme.js';
import { RefreshModelStateOfProvider } from '../../../../../../../workbench/contrib/mcode/common/refreshModelService.js';
...
import { IRefreshModelService } from '../../../../../../../workbench/contrib/mcode/common/refreshModelService.js';
import { IVoidSettingsService } from '../../../../../../../workbench/contrib/mcode/common/mcodeSettingsService.js';
import { IExtensionTransferService } from '../../../../../../../workbench/contrib/mcode/browser/extensionTransferService.js';
...
import { IMetricsService } from '../../../../../../../workbench/contrib/mcode/common/metricsService.js';
...
import { IVoidRagService, RagIndexStatus, defaultRagIndexStatus } from '../../../../common/mcodeRagTypes.js';
...
let ragIndexState: RagIndexStatus = defaultRagIndexStatus();
const ragIndexStateListeners: Set<(s: RagIndexStatus) => void> = new Set();
...
export const _registerServices = (accessor: ServicesAccessor) => {
  ...
  const stateServices = {
    ...
    mcodeCommandBarService: accessor.get(IMcodeCommandBarService),
    modelService: accessor.get(IModelService),
    mcpService: accessor.get(IMCPService),
    ragService: accessor.get(IVoidRagService)
  };

  const { settingsStateService, chatThreadsStateService, refreshModelService, themeService, editCodeService, mcodeCommandBarService, modelService, mcpService, ragService } = stateServices;
  ...
  disposables.push(
    mcodeCommandBarService.onDidChangeState(({ uri }) => {
      commandBarURIStateListeners.forEach((l) => l(uri));
    })
  );

  disposables.push(
    mcodeCommandBarService.onDidChangeActiveURI(({ uri }) => {
  ...
  const applyRagIndexStatus = (status: RagIndexStatus) => {
    ragIndexState = status;
    ragIndexStateListeners.forEach((l) => l(ragIndexState));
  };

  void ragService.getIndexStatus().then(applyRagIndexStatus).catch(() => {});

  disposables.push(
    ragService.onIndexProgress((e) => {
      applyRagIndexStatus({
        ...ragIndexState,
        phase: e.phase,
        filesDone: e.filesDone,
        filesTotal: e.filesTotal,
        fileCount: e.filesDone,
        chunkCount: e.chunks,
        currentFile: e.currentFile ?? null,
        error: null
      });
    })
  );
  disposables.push(
    ragService.onIndexComplete((e) => {
      applyRagIndexStatus({
        ...ragIndexState,
        phase: 'idle',
  ...
  disposables.push(
    ragService.onIncrementalSync((e) => {
      applyRagIndexStatus({
        ...ragIndexState,
        lastIncrementalSync: e
      });
    })
  );
...
const getReactAccessor = (accessor: ServicesAccessor) => {
  const reactAccessor = {
    ...
    ITerminalPtyService: accessor.get(ITerminalPtyService),
    ...
    IMcodeCommandBarService: accessor.get(IMcodeCommandBarService),
    ...
    IContextGatheringService: accessor.get(IContextGatheringService),
...
export const useCommandBarState = () => {
  ...
  const commandBarService = accessor.get('IMcodeCommandBarService');
...
export const useRagIndexState = () => {
  const [s, ss] = useState(ragIndexState);
  useEffect(() => {
    ragIndexStateListeners.add(ss);
    return () => {ragIndexStateListeners.delete(ss);};
  }, [ss]);
  return s;
};
