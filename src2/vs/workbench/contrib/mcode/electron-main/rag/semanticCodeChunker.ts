...
type LanguageFamily = 'cpp' | 'typescript' | 'javascript' | 'python' | 'scilab' | 'matlab' | 'java' | 'unknown';
...
function chunkScilab(content: string): SemanticCodeChunk[] {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		let end = i + 1;
		while (end < lines.length && !/^\s*endfunction\b/i.test(lines[end])) {
			end++;
		}
		if (end < lines.length) {
			end++;
...
export async function chunkCodeForIndexing(content: string, filePath: string, maxSymbolLines = MAX_SYMBOL_LINES): Promise<SemanticCodeChunk[]> {
	...
	let chunks: SemanticCodeChunk[];

	const { canTreeSitterParse, chunkWithTreeSitter } = await import('./treeSitterChunker.js');
	if (canTreeSitterParse(filePath)) {
		try {
			const astChunks = await chunkWithTreeSitter(sliceContent, filePath);
